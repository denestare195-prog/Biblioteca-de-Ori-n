...
def obtener_imagen_src(ruta):
    ...
    # Imagen de respaldo por defecto si la portada no se encuentra localmente
...
# Inicializar el estado de la sesión con tus libros originales y la categoría "Realidad Nacional"
...
# Menú lateral (Sin la sección de sugerir aporte)
...
if menu == "Ver Biblioteca":
    ...
    if not libros_filtrados:
        ...
        # Extraer categorías únicas (por ahora solo "Realidad Nacional", preparado para más adelante)
        ...
        for idx, categoria in enumerate(categorias):
            ...
            # Renderizar carrusel horizontal con botones de desplazamiento en bucle
            st.markdown(f"""
...
# -------------------------------------------------------------
    ...
    st.header("Panel de Moderación y Libros Registrados")
    st.write("Lista actual de libros en la base de datos:")
    
    for libro in st.session_state.libros:
        st.write(f"- **{libro['titulo']}** | *{libro['autor']}* (Categoría: {libro['categoria']})")
